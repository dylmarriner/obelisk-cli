...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@obelisk-ai/sdk/v2"
...
export const AuthMethodID = "obelisk-login"
...
export class Service extends Context.Service<Service, Interface>()("@obelisk/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `obelisk auth login` in the terminal",
      name: "Login with obelisk",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, obelisk provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const obeliskProvider = providers[ProviderV2.ID.make("obelisk")]
  const obeliskModel = obeliskProvider ? Provider.sort(Object.values(obeliskProvider.models))[0] : undefined
  if (obeliskProvider && obeliskModel) return { providerID: obeliskProvider.id, modelID: obeliskModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Obelisk service failure", service })
}
