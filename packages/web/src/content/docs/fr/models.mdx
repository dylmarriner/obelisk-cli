...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Obelisk.
...
Voici plusieurs modèles qui fonctionnent bien avec Obelisk, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Obelisk.

```json title="obelisk.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Obelisk Zen](/docs/zen), vous utiliserez `obelisk/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="obelisk.jsonc" {7-12,19-24}
{
  "$schema": "https://obelisk.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="obelisk.jsonc" {6-21}
{
  "$schema": "https://obelisk.ai/config.json",
  "provider": {
    "obelisk": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Obelisk est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Obelisk est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="obelisk.jsonc" {7-18}
{
  "$schema": "https://obelisk.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Obelisk démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Obelisk.
