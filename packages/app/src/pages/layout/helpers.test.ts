...
import { type Session } from "@obelisk-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("obelisk://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("obelisk://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("obelisk://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("obelisk://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("obelisk://open-project")).toBeUndefined()
    expect(parseDeepLink("obelisk://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "obelisk://open-project?directory=/a",
      "obelisk://other?directory=/b",
      "obelisk://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("obelisk://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("obelisk://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("obelisk://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("obelisk://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "obelisk://new-session?directory=/a",
      "obelisk://open-project?directory=/b",
      "obelisk://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OBELISK__: {
        deepLinks: ["obelisk://open-project?directory=/a"],
      },
    } as unknown as Window & { __OBELISK__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["obelisk://open-project?directory=/a"])
