# Remove `packages/obelisk/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/obelisk/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the obelisk legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/obelisk/src/storage/db.ts` are concentrated in 22 source files:

- `packages/obelisk/src/account/repo.ts`
- `packages/obelisk/src/cli/cmd/db.ts`
- `packages/obelisk/src/cli/cmd/import.ts`
- `packages/obelisk/src/cli/cmd/stats.ts`
- `packages/obelisk/src/control-plane/workspace.ts`
- `packages/obelisk/src/index.ts`
- `packages/obelisk/src/node.ts`
- `packages/obelisk/src/permission/index.ts`
- `packages/obelisk/src/project/project.ts`
- `packages/obelisk/src/server/projectors.ts`
- `packages/obelisk/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/obelisk/src/server/shared/fence.ts`
- `packages/obelisk/src/session/message-v2.ts`
- `packages/obelisk/src/session/projectors.ts`
- `packages/obelisk/src/session/prompt.ts`
- `packages/obelisk/src/session/session.ts`
- `packages/obelisk/src/session/todo.ts`
- `packages/obelisk/src/share/share-next.ts`
- `packages/obelisk/src/storage/db.ts`
- `packages/obelisk/src/sync/index.ts`
- `packages/obelisk/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy obelisk database wrapper; `packages/obelisk/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the obelisk wrapper.
...
Status: Completed. `SyncEvent` and the obelisk projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy obelisk database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy obelisk database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy obelisk database wrapper.
...
All migration groups are complete or superseded. `packages/obelisk/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No obelisk data-migration group remains.

The previous obelisk `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate obelisk data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/obelisk`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/obelisk/src`
- `bun typecheck` from `packages/obelisk`
- Relevant package tests from `packages/obelisk`, not the repo root
