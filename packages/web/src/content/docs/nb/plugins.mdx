...
description: Skriv dine egne programtillegg for å utvide Obelisk.
---

Plugins lar deg utvide Obelisk ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Obelisk.
...
- `.obelisk/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/obelisk/node_modules/`.
...
1. Global konfigurasjon (`~/.config/obelisk/obelisk.json`)
2. Prosjektkonfigurasjon (`obelisk.json`)
3. Global plugin-katalog (`~/.config/obelisk/plugins/`)
4. Prosjektpluginkatalog (`.obelisk/plugins/`)
...
Obelisk kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Obelisk SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Obelisk.
...
:::note
Hvis du bruker Obelisk-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Obelisk fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Obelisk:
...
`tool`-hjelperen lager et tilpasset verktøy som Obelisk kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Obelisk sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://obelisk.ai/docs/sdk) for detaljer.
