...
Agentferdigheter lar Obelisk oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Obelisk søker etter disse stedene:

- Prosjektkonfigurasjon: `.obelisk/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/obelisk/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Obelisk opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.obelisk/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.obelisk/skills/git-release/SKILL.md` slik:
...
Obelisk viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `obelisk.json`:
...
**For innebygde agenter** (i `obelisk.json`):
