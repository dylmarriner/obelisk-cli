...
With Obelisk you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Obelisk uses our own `obelisk` theme.
...
Obelisk comes with several built-in themes.
...
- Want Obelisk to match their terminal's appearance
...
Obelisk supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/obelisk/themes/*.json` or `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.obelisk/themes/*.json`
4. **Current working directory** - Defined in `./.obelisk/themes/*.json`
