import type { PermissionV1 } from "@obelisk-ai/core/v1/permission"
import { FSUtil } from "@obelisk-ai/core/fs-util"
// CLI entry point for `obelisk run` and `obelisk --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`obelisk --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`obelisk --mini --attach`): connects to a running
//      obelisk server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@obelisk-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run obelisk with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running obelisk server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OBELISK_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OBELISK_SERVER_USERNAME or 'obelisk')",
      })
      .option("dir", {
        type: "string",
