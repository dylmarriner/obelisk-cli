...
Det er to typer agenter i Obelisk; primære agenter og underagenter.
...
Obelisk kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Obelisk kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Obelisk kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `obelisk.json`:
...
Hvis ingen temperatur er spesifisert, bruker Obelisk modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Obelisk-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Obelisk-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Obelisk Zen](/docs/zen), vil du bruke `obelisk/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `obelisk models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/obelisk).
:::
