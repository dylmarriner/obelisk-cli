...
This is the living design and status document for `@obelisk-ai/codemode` and its existing V2 Obelisk adapter.
...
`packages/obelisk/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
