---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Obelisk zu erweitern.
---

Mit Plugins können Sie Obelisk erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Obelisk zu ändern.
...
- `.obelisk/plugins/` – Plugins auf Projektebene
- `~/.config/obelisk/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/obelisk/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/obelisk/obelisk.json`)
2. Projektkonfiguration (`obelisk.json`)
3. Globales Plugin-Verzeichnis (`~/.config/obelisk/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.obelisk/plugins/`)
...
Obelisk führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Obelisk-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Obelisk erweitern können.
...
:::note
Wenn Sie die Obelisk-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Obelisk `.env`-Dateien liest:
...
Plugins können Obelisk auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Obelisk aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Obelisk zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://obelisk.ai/docs/sdk).
