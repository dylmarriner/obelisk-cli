...
Move the canonical Obelisk terminal application from
`packages/obelisk/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/obelisk`,
`packages/cli`, or `@obelisk-ai/core`.

The SDK is the TUI's Obelisk boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/obelisk`.
- Do not replace private `packages/obelisk` imports with `@obelisk-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@obelisk-ai/tui`.
...
- Obelisk domain data displayed by the TUI
...
```text
packages/obelisk/src/cli/cmd/tui
```

Its private dependency on `packages/obelisk` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/obelisk/src/*`.
...
Status: Completed. The private `@obelisk-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@obelisk-ai/tui`.
...
- The package has no dependency on `obelisk`, `@obelisk-ai/cli`, or
  `@obelisk-ai/core`.
...
primitives now live in `@obelisk-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@obelisk-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Obelisk globals or process state;
...
Replace process-global Obelisk state with resolved TUI inputs.
...
`@obelisk-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/obelisk` initially.
...
canonical in `@obelisk-ai/tui`. Configured references resolve through the new
...
adapters rather than Obelisk domain implementations.

Make the SDK the only Obelisk domain boundary used by the TUI.
...
- Domain-facing TUI code imports Obelisk data and operations only from
  `@obelisk-ai/sdk`.
...
`@obelisk-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/obelisk`.
...
- The package has no imports from `packages/obelisk`, `packages/cli`, or
  `@obelisk-ai/core`.
...
public `@obelisk-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/obelisk`.
- Keep the legacy embedded worker and server startup in `packages/obelisk`.
- Change those adapters to load config, create transport inputs, and call the
  public `@obelisk-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/obelisk`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@obelisk-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@obelisk-ai/core`, or either
  executable package.
...
rg 'src/cli/cmd/tui|@tui/' packages/obelisk/src packages/obelisk/test
```
...
- Build the current-platform `packages/obelisk` binary.
