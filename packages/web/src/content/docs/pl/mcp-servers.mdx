...
Możesz dodać dodatkowe informacje do obelisk za pomocą _Model Context Protocol_, w skrócie MCP. obelisk obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [obelisk Config](https://obelisk.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/obelisk`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
obelisk automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, obelisk:
...
Jeśli serwer wymaga uwierzytelnienia, obelisk poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `obelisk mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji obelisk bezpieczne przechowa tokeny w `~/.local/share/obelisk/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w obelisk, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie obelisk, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć obelisk z kontem Sentry.
