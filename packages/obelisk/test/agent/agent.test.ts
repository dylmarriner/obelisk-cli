...
it.instance("plan agent denies edits except .obelisk/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".obelisk/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".obelisk", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OBELISK_TEST_HOME
      process.env.OBELISK_TEST_HOME = test.directory
