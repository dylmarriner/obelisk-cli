...
export const DEPRECATED_PLUGIN_PACKAGES = ["obelisk-openai-codex-auth", "obelisk-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, obeliskVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(obeliskVersion) || semver.major(obeliskVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.obelisk
  if (typeof range !== "string") return
  if (!semver.satisfies(obeliskVersion, range)) {
    throw new Error(`Plugin requires obelisk ${range} but running ${obeliskVersion}`)
  }
}
