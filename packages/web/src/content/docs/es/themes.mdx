...
Con Obelisk puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Obelisk usa nuestro propio tema `obelisk`.
...
Obelisk viene con varios temas integrados.
...
- Quiere que Obelisk coincida con la apariencia de su terminal
...
Obelisk admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/obelisk/themes/*.json` o `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.obelisk/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.obelisk/themes/*.json`
