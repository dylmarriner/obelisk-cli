...
Agent skills omogucavaju Obelisk da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Obelisk pretrazuje ove lokacije:

- Konfiguracija projekta: `.obelisk/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/obelisk/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Obelisk ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.obelisk/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.obelisk/skills/git-release/SKILL.md` ovako:
...
Obelisk navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `obelisk.json`:
...
**Za ugradene agente** (u `obelisk.json`):
