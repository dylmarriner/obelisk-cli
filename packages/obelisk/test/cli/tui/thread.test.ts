...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ obelisk }) =>
    Effect.gen(function* () {
      const result = yield* obelisk.spawn(["--replay-limit", "10"])

      obelisk.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ obelisk }) =>
    Effect.gen(function* () {
      const result = yield* obelisk.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ obelisk }) =>
    Effect.gen(function* () {
      const result = yield* obelisk.spawn(["--mini", "--port", "4096"])
