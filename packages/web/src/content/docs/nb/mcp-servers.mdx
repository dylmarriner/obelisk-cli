...
Du kan legge til eksterne verktøy i Obelisk ved å bruke _Model Context Protocol_ eller MCP. Obelisk støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Obelisk Config](https://obelisk.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="obelisk.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/obelisk`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Obelisk håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Obelisk:
...
Hvis serveren krever autentisering, vil Obelisk be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `obelisk mcp auth <server-name>`.
...
```bash
obelisk mcp auth my-oauth-server
```
...
```bash
obelisk mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Obelisk lagre tokens sikkert i `~/.local/share/obelisk/mcp-auth.json`.
...
obelisk mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Obelisk, sammen med innebygde verktøy. Så du kan administrere dem gjennom Obelisk-konfigurasjonen som alle andre verktøy.
...
```json title="obelisk.json" {11, 14-18}
{
  "$schema": "https://obelisk.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
obelisk mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Obelisk til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Obelisk-samtale oppført som en separat økt.
