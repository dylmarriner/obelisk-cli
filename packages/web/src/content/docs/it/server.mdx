---
title: Server
description: Interagisci con il server obelisk via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `obelisk serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client obelisk.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `obelisk.local` |
...
Imposta `OBELISK_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `obelisk`, oppure imposta `OBELISK_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `obelisk serve` sia per `obelisk web`.
...
Quando esegui `obelisk` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server obelisk per interagire con obelisk in modo programmatico.
:::

Questa architettura permette a obelisk di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `obelisk serve` per avviare un server standalone. Se la TUI di obelisk e gia in esecuzione, `obelisk serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Obelisk.
...
Il server obelisk espone le seguenti API.
