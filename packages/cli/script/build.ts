...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      OBELISK_CLI_NAME: `'${binary}'`,
      OBELISK_MODELS_DEV: modelsData,
      OBELISK_CHANNEL: `'${Script.channel}'`,
      OBELISK_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/obelisk.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
