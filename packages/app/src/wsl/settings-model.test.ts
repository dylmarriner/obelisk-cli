...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Obelisk needs attention", () => {
    ...
    ).toBe("Install Obelisk")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/obelisk",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Obelisk is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        obeliskChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.obelisk/bin/obelisk",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-obelisk")
  })
