...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.OBELISK_TEST_HOME
      process.env.OBELISK_TEST_HOME = home
      return prev
    }),
    () => self,
    (prev) =>
      Effect.sync(() => {
        process.env.OBELISK_TEST_HOME = prev
      }),
  )
...
describe("skill", () => {
  ...
  it.live("discovers skills from .obelisk/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".obelisk", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".obelisk", "skill", "dir-skill", "SKILL.md"),
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".obelisk", "skill", "dir-skill"))
  ...
  it.live("discovers multiple skills from .obelisk/skill/ directory", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".obelisk", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".obelisk", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".obelisk", "skill", "no-frontmatter", "SKILL.md"),
              `# No Frontmatter
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".obelisk", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".obelisk", "skill", "obelisk-skill", "SKILL.md"),
                `---
name: obelisk-skill
description: A skill in the .obelisk/skill directory.
---
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["obelisk-skill"])
        }),
      { git: true },
    ),
  )
...
`,
              ...
              Bun.write(
                path.join(dir, ".obelisk", "skill", "agent-skill", "SKILL.md"),
                `---
name: obelisk-skill
description: A skill in the .obelisk/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".obelisk", "skills", "agent-skill", "SKILL.md"),
                `---
name: obelisk-skill
description: A skill in the .obelisk/skills directory.
---
