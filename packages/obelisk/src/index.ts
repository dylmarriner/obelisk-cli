...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    if (opts.printLogs) process.env.OBELISK_PRINT_LOGS = "1"
    if (opts.logLevel) process.env.OBELISK_LOG_LEVEL = opts.logLevel
    if (opts.pure) {
    ...
    process.env.AGENT = "1"
    process.env.OBELISK = "1"
    process.env.OBELISK = "1"
    process.env.OBELISK_PID = String(process.pid)
  })
  .usage("")
