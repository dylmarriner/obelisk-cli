...
Per diagnosticare problemi con Obelisk, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\obelisk\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `obelisk --log-level DEBUG`.

---

## Archiviazione

obelisk salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\obelisk`
...
Obelisk Desktop esegue in background un server locale di Obelisk (il sidecar `obelisk-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Obelisk` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
Obelisk puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\obelisk\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Obelisk possa ricostruirla.

1. Chiudi completamente Obelisk Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/obelisk`
- **Linux**: elimina `~/.cache/obelisk` (oppure esegui `rm -rf ~/.cache/obelisk`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\obelisk`

3. Riavvia Obelisk Desktop.

---

### Risolvi problemi di connessione al server

Obelisk Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `obelisk.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OBELISK_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OBELISK_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Obelisk Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Obelisk.

---

### Notifiche non visualizzate

Obelisk Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Obelisk nelle impostazioni del sistema operativo, e
...
1. Chiudi Obelisk Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Obelisk Desktop):

- `obelisk.settings.dat` (desktop default server URL)
- `obelisk.global.dat` e `obelisk.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Obelisk:
...
Per capire a quali modelli hai accesso, esegui `obelisk models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\obelisk`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. obelisk installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\obelisk`

2. Riavvia obelisk per reinstallare i pacchetti provider piu recenti

Questo forzera obelisk a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
obelisk rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
