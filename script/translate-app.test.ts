...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".obelisk/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".obelisk/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".obelisk/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "obelisk/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "obelisk/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "obelisk/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
