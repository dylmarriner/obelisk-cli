...
import { createOpencode } from "@obelisk-ai/sdk"
...
`)
  ...
  const obelisk = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await obelisk.client.session.create()
    const result = await obelisk.client.session
      .prompt({
