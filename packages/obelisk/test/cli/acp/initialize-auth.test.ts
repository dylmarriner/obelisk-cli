...
describe("obelisk acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ obelisk }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ obelisk }))
        ...
        expect(initialized.agentInfo?.name).toBe("Obelisk")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ obelisk })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("obelisk-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "obelisk-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OBELISK_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("obelisk-login")
