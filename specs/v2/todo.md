...
The obelisk server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@obelisk-ai/llm` tool loop after replacing its
...
- plugins get global "obelisk" instance like in that post i showed
- obelisk instance has stuff like `obelisk.session.prompt()` or
  `obelisk.tool.register({...})`
...
- decide whether to preserve deprecated `@obelisk-ai/llm` orchestration exports
