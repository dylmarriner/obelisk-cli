...
Com o obelisk, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o obelisk usa nosso próprio tema `obelisk`.
...
O obelisk vem com vários temas embutidos.
...
- Querem que o obelisk corresponda à aparência do seu terminal
...
O obelisk suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/obelisk/themes/*.json` ou `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.obelisk/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.obelisk/themes/*.json`
