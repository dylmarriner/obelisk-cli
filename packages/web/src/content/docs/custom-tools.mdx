---
title: Custom Tools
description: Create tools the LLM can call in obelisk.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside obelisk's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.obelisk/tools/` directory of your project.
- Or globally, by placing them in `~/.config/obelisk/tools/`.
