...
Dzięki obelisk możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie obelisk używa naszego własnego motywu `obelisk`.
...
obelisk ma kilka wbudowanych motywów.
...
- Chcą, aby Obelisk pasował do wyglądu ich terminala
...
obelisk obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/obelisk/themes/*.json` lub `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.obelisk/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.obelisk/themes/*.json`
