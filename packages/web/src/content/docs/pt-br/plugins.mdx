---
title: Plugins
description: Escreva seus próprios plugins para estender o obelisk.
---

Plugins permitem que você estenda o obelisk conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do obelisk.
...
- `.obelisk/plugins/` - Plugins em nível de projeto
- `~/.config/obelisk/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/obelisk/node_modules/`.
...
1. Configuração global (`~/.config/obelisk/obelisk.json`)
2. Configuração do projeto (`obelisk.json`)
3. Diretório de plugins global (`~/.config/obelisk/plugins/`)
4. Diretório de plugins do projeto (`.obelisk/plugins/`)
...
O obelisk executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do obelisk para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o obelisk.
...
:::note
Se você estiver usando o aplicativo desktop obelisk, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o obelisk de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao obelisk:
...
O helper `tool` cria uma ferramenta personalizada que o obelisk pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o obelisk junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://obelisk.ai/docs/sdk) para detalhes.
