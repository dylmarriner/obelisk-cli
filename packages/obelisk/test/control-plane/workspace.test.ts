...
import { EventSequenceTable } from "@obelisk-ai/core/event/sql"
...
import { WorkspaceV2 } from "@obelisk-ai/core/workspace"
import { WorkspaceTable } from "@obelisk-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@obelisk-ai/core/ripgrep"
...
const originalEnv = {
  OBELISK_AUTH_CONTENT: process.env.OBELISK_AUTH_CONTENT,
  OBELISK_EXPERIMENTAL_WORKSPACES: process.env.OBELISK_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OBELISK_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@obelisk.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OBELISK_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=obelisk-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OBELISK_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OBELISK_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OBELISK_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=obelisk-test")
