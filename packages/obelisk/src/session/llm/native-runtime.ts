...
import type { LLMClientShape } from "@obelisk-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("obelisk"))
    return { type: "unsupported", reason: "provider is not openai, obelisk, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @obelisk-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains obelisk-owned. The native runtime only adapts
      // the @obelisk-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
