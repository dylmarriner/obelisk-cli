...
Umiejętności agenta pozwalają obelisk odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
obelisk przeszukuje te lokalizacje:

- Project config: `.obelisk/skills/<name>/SKILL.md`
- Global config: `~/.config/obelisk/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu obelisk przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.obelisk/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.obelisk/skills/git-release/SKILL.md` w ten sposób:
...
obelisk wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `obelisk.json`:
...
**Dla agentów wbudowanych** (w `obelisk.json`):
