...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "obelisk-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["OBELISK_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { OBELISK_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("OBELISK_TEST_SHELL=ok")
      }),
    )
