---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Obelisk.
---

Dodaci vam omogućavaju da proširite Obelisk spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Obelisk.
...
- `.obelisk/plugins/` - Dodaci na nivou projekta
- `~/.config/obelisk/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/obelisk/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/obelisk/obelisk.json`)
2. Konfiguracija projekta (`obelisk.json`)
3. Globalni direktorij dodataka (`~/.config/obelisk/plugins/`)
4. Direktorij dodataka projekta (`.obelisk/plugins/`)
...
Obelisk pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Obelisk.
...
Spriječite obelisk da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u obelisk:
...
Pomoćnik `tool` kreira prilagođeni alat koji obelisk može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za obelisk zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://obelisk.ai/docs/sdk) za detalje.
