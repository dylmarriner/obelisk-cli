---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

Obelisk verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Obelisk-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Obelisk anpassen
config.
...
Obelisk Zen ist eine Liste von Modellen, die vom Obelisk-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Obelisk funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Obelisk Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Obelisk aus und gehen Sie zu [obelisk.ai/auth](https://obelisk.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Obelisk und ist völlig optional.

---

## Obelisk Go

Obelisk Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Obelisk-Team getestet und verifiziert wurden, dass sie gut mit Obelisk funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Obelisk Go` und gehen Sie zu [obelisk.ai/auth](https://obelisk.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Obelisk:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Obelisk ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `obelisk.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Obelisk wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können obelisk so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Obelisk ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Obelisk-Konfiguration hinzufügen.
...
um das Token im Obelisk-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Obelisk verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Obelisk
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`obelisk.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `obelisk.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Obelisk:
...
So verwenden Sie Google Vertex AI mit Obelisk:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Obelisk ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Obelisk konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session)-Plugin, um jede Obelisk-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Obelisk-Konversation als separate Sitzung aufgeführt.
...
Sie können Obelisk für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Obelisk über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Obelisk über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Obelisk konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/obelisk).
:::
...
So verwenden Sie Ollama Cloud mit Obelisk:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Obelisk.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Obelisk verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Obelisk Zen ist eine Liste getesteter und verifizierter Modelle, die vom Obelisk-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Obelisk Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Obelisk Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Obelisk API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Obelisk-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Obelisk-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Obelisk:
...
Sie können Modelle auch über Ihre Obelisk-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Obelisk verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `obelisk.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Obelisk erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `obelisk auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Obelisk-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Obelisk-Konfiguration übereinstimmt.
