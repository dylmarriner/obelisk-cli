...
// terminal sizes. The default obelisk tui command is excluded —
// `obelisk --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `obelisk --help` prints today;
...
// top-level help on `--help` and exits 1; not a real obelisk command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("obelisk CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const topLevel = yield* obelisk.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* obelisk.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`obelisk ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`obelisk ${argv.join(" ")} --help`)
        }
