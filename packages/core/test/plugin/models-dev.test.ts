...
import { ModelsDevPlugin } from "@obelisk-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.OBELISK_MODELS_PATH,
          disabled: Flag.OBELISK_DISABLE_MODELS_FETCH,
        }
        Flag.OBELISK_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.OBELISK_MODELS_PATH = previous.path
          Flag.OBELISK_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
