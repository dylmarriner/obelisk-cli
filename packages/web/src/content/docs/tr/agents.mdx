...
obelisk'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
obelisk, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Obelisk, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Obelisk iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `obelisk.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse obelisk modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global obelisk yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
obelisk yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Obelisk Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `obelisk/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `obelisk models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/obelisk).
:::
