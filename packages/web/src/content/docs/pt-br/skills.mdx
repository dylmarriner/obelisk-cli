...
As habilidades do agente permitem que o obelisk descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O obelisk pesquisa nesses locais:

- Configuração do projeto: `.obelisk/skills/<name>/SKILL.md`
- Configuração global: `~/.config/obelisk/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o obelisk sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.obelisk/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.obelisk/skills/git-release/SKILL.md` assim:
...
O obelisk lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `obelisk.json`:
...
**Para agentes embutidos** (em `obelisk.json`):
