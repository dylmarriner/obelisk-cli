...
U Obelisk mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Obelisk koristi nasu `obelisk` temu.
...
Obelisk dolazi sa vise ugradenih tema.
...
- Zele da Obelisk odgovara izgledu njihovog terminala
...
Obelisk podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/obelisk/themes/*.json` ili `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.obelisk/themes/*.json`
4. **Trenutni radni direktorij** - `./.obelisk/themes/*.json`
