---
title: Server
description: Nutze den obelisk-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `obelisk serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein obelisk-Client nutzen kann.
...
Setze `OBELISK_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `obelisk`, kann aber mit `OBELISK_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `obelisk serve` und `obelisk web`.
...
Wenn du `obelisk` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den obelisk-Server, um obelisk programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `obelisk serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `obelisk serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Obelisk-[IDE](/docs/ide)-Plugins.
...
Der obelisk-Server stellt folgende APIs bereit.
