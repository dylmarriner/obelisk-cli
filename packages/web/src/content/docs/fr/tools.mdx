...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Obelisk est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Obelisk.
...
:::note
Cet outil n'est disponible que lorsque `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (ou `OBELISK_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Obelisk ou lorsque la variable d'environnement `OBELISK_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Obelisk :
