---
title: Server
description: Interager med obelisk-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `obelisk serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en obelisk-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `obelisk.local` |
...
Indstil `OBELISK_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `obelisk`, eller indstil `OBELISK_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `obelisk serve` og `obelisk web`.
...
Når du kører `obelisk` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug obelisk-serveren til at interagere med obelisk programmatisk.
:::

Denne arkitektur lader obelisk understøtte flere klienter og lader dig interagere med obelisk programmatisk.

Du kan køre `obelisk serve` for at starte en selvstændig server. Hvis du har
obelisk TUI kørende, vil `obelisk serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Obelisk [IDE](/docs/ide) plugins.
...
Obelisk-serveren viser følgende API'er.
