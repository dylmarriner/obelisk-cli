---
title: Config
description: Using the Obelisk JSON config.
---

You can configure Obelisk using a JSON config file.

---

## Format

Obelisk supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="obelisk.jsonc"
{
...
1. **Remote config** (from `.well-known/obelisk`) - organizational defaults
2. **Global config** (`~/.config/obelisk/obelisk.json`) - user preferences
3. **Custom config** (`OBELISK_CONFIG` env var) - custom overrides
4. **Project config** (`obelisk.json` in project) - project-specific settings
5. **`.obelisk` directories** - agents, commands, plugins
6. **Inline config** (`OBELISK_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/obelisk/` on macOS) - admin-controlled
...
:::note
The `.obelisk` and `~/.config/obelisk` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/obelisk` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/obelisk"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Obelisk config in `~/.config/obelisk/obelisk.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/obelisk/tui.json`.
...
Add `obelisk.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Obelisk starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `OBELISK_CONFIG` environment variable.

```bash
export OBELISK_CONFIG=/path/to/my/custom-config.json
obelisk run "Hello world"
```
...
Specify a custom config directory using the `OBELISK_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.obelisk` directory, and should
follow the same structure.

```bash
export OBELISK_CONFIG_DIR=/path/to/my/config-directory
obelisk run "Hello world"
```

The custom directory is loaded after the global config and `.obelisk` directories, so it **can override** their settings.
...
Drop an `obelisk.json` or `obelisk.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/obelisk/` |
| Linux    | `/etc/obelisk/`                         |
| Windows  | `%ProgramData%\obelisk`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Obelisk reads managed preferences from the `ai.obelisk.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Obelisk checks these paths:

1. `/Library/Managed Preferences/<user>/ai.obelisk.managed.plist`
2. `/Library/Managed Preferences/ai.obelisk.managed.plist`

The plist keys map directly to `obelisk.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.obelisk.managed` PayloadType. The Obelisk config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.obelisk.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.obelisk.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.obelisk</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`obelisk.ai/config.json`**](https://obelisk.ai/config.json).

TUI config uses [**`obelisk.ai/tui.json`**](https://obelisk.ai/tui.json).
...
Use `OBELISK_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `obelisk.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `obelisk serve` and `obelisk web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Obelisk server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `obelisk.local`. Useful for running multiple instances on the same network.
...
If not specified, Obelisk will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Obelisk config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Obelisk tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Obelisk actions on configured resources. Currently, policies can control which providers Obelisk may use.
...
Obelisk normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Obelisk omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/obelisk/agents/` or `.obelisk/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Obelisk will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`obelisk run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/obelisk/commands/` or `.obelisk/commands/`. [Learn more here](/docs/commands).
...
Obelisk uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Obelisk will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, obelisk **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Obelisk with custom tools, hooks, and integrations.

Place plugin files in `.obelisk/plugins/` or `~/.config/obelisk/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="obelisk.json"
{
  "$schema": "https://obelisk.ai/config.json",
  "plugin": ["obelisk-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Obelisk to only use specific providers rather than disabling them one by one.
