---
title: Server
description: Interact with obelisk server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `obelisk serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an obelisk client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `obelisk.local` |
...
Set `OBELISK_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `obelisk`, or set `OBELISK_SERVER_USERNAME` to override it. This applies to both `obelisk serve` and `obelisk web`.
...
When you run `obelisk` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the obelisk server to interact with obelisk programmatically.
:::

This architecture lets obelisk support multiple clients and allows you to interact with obelisk programmatically.

You can run `obelisk serve` to start a standalone server. If you have the
obelisk TUI running, `obelisk serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Obelisk [IDE](/docs/ide) plugins.
...
The obelisk server exposes the following APIs.
