...
Para depurar problemas con Obelisk, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\obelisk\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `obelisk --log-level DEBUG`.

---

## Almacenamiento

obelisk almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\obelisk`
...
Obelisk Desktop ejecuta un servidor Obelisk local (el sidecar `obelisk-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Obelisk` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/obelisk/obelisk.jsonc` (o `~/.config/obelisk/obelisk.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
Obelisk también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\obelisk\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Obelisk pueda reconstruirlo.

1. Salga de Obelisk Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/obelisk`
- **Linux**: eliminar `~/.cache/obelisk` (o ejecutar `rm -rf ~/.cache/obelisk`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\obelisk`

3. Reinicie el escritorio Obelisk.

---

### Solucionar problemas de conexión del servidor

Obelisk Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `obelisk.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OBELISK_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OBELISK_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Obelisk requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Obelisk.

---

### Notificaciones que no se muestran

Obelisk Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Obelisk en la configuración de su sistema operativo, y
...
1. Salga del escritorio Obelisk.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Obelisk):

- `obelisk.settings.dat` (URL del servidor predeterminado de escritorio)
- `obelisk.global.dat` y `obelisk.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Obelisk:
...
3. Asegúrese de tener la última versión con `obelisk upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `obelisk models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\obelisk`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. obelisk instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\obelisk`

2. Reinicie obelisk para reinstalar los paquetes del proveedor más recientes.

Esto obligará a obelisk a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
obelisk detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
