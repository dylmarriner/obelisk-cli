// Subprocess integration tests for `obelisk run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `obelisk.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `OBELISK_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("obelisk run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("say hi")
        obelisk.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* obelisk.run("think", { extraArgs: ["--thinking"] })
        obelisk.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* obelisk.run("think again")
        obelisk.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("say hi", { format: "json" })
        obelisk.expectExit(result, 0)

        const events = obelisk.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const result = yield* obelisk.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const result = yield* obelisk.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* obelisk.run("request permission", { permission: { bash: "ask" } })
        obelisk.expectExit(denied, 0)
        ...
        const allowed = yield* obelisk.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        obelisk.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* obelisk.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        obelisk.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, obelisk }) =>
      Effect.gen(function* () {
        ...
        const server = yield* obelisk.serve()

        const result = yield* obelisk.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, obelisk }) =>
      Effect.gen(function* () {
        const result = yield* obelisk.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, obelisk }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* obelisk.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
