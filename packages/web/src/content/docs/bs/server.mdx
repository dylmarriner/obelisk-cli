---
title: Server
description: Komunicirajte s obelisk serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `obelisk serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji obelisk klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `obelisk.local` |
...
Postavite `OBELISK_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `obelisk`, ili postavite `OBELISK_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `obelisk serve` i za `obelisk web`.
...
Kada pokrenete `obelisk`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite obelisk server za programsku interakciju sa obelisk-om.
:::

Ova arhitektura omogucava obelisk podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `obelisk serve` da startate standalone server. Ako je obelisk TUI vec pokrenut, `obelisk serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Obelisk [IDE](/docs/ide) pluginovi.
...
obelisk server izlaže sljedece API-je.
