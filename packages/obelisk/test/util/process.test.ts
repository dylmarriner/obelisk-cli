...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OBELISK_TEST ?? "")'), {
      env: {
        OBELISK_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OBELISK_TEST_SHELL"], {
      shell: true,
      env: {
        OBELISK_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OBELISK_TEST_SHELL=ok")
  })
