...
Avec Obelisk, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Obelisk utilise notre propre thème `obelisk`.
...
Obelisk est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Obelisk corresponde à l'apparence de leur terminal
...
Obelisk prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/obelisk/themes/*.json` ou `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.obelisk/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.obelisk/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/obelisk/themes
vim ~/.config/obelisk/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .obelisk/themes
vim .obelisk/themes/my-theme.json
```
...
{
  "$schema": "https://obelisk.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
