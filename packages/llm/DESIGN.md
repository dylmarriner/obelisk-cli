...
> current private `@obelisk-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@obelisk-ai/ai`
...
- Secondary audience: Obelisk and other durable agent runtimes
...
import { LLM } from "@obelisk-ai/ai"
import { OpenAI } from "@obelisk-ai/ai/providers/openai"
...
```ts
import { Message } from "@obelisk-ai/ai"
...
import { LLM, Tool } from "@obelisk-ai/ai"
...
Obelisk and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@obelisk-ai/ai/promise"
import { OpenAI } from "@obelisk-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@obelisk-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@obelisk-ai/ai/provider"
...
@obelisk-ai/ai/promise
  Promise/AsyncIterable LLM facade

@obelisk-ai/ai/schema
  serializable domain schemas

@obelisk-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@obelisk-ai/ai/providers/openai
@obelisk-ai/ai/providers/anthropic
@obelisk-ai/ai/providers/google
...
```
...
Obelisk should migrate to `generateTurn` / `streamTurn`, preserving its durable
