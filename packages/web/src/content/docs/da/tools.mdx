...
Her er alle de innebygde verktøyene som er tilgængelige i Obelisk.
...
:::note
Dette verktøyet er bare tilgængelig når `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (eller `OBELISK_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Obelisk-leverandøren eller når miljøvariabelen `OBELISK_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
