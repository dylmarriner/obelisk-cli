...
Možete dodati vanjske alate u Obelisk koristeći _Model Context Protocol_, ili MCP. Obelisk podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Obelisk Config](https://obelisk.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/obelisk`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Obelisk automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Obelisk će:
...
Ako server zahtijeva autentifikaciju, Obelisk će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `obelisk mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Obelisk sigurno čuva tokene u `~/.local/share/obelisk/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Obelisk, zajedno s ugrađenim alatima. Možete njima upravljati kroz Obelisk konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Obelisk sa vašim Sentry nalogom.
