...
description: Usa la configurazione JSON di Obelisk.
---

Puoi configurare Obelisk usando un file di configurazione JSON.

---

## Formato

Obelisk supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/obelisk`) - default dell'organizzazione
2. **Config globale** (`~/.config/obelisk/obelisk.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OBELISK_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`obelisk.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.obelisk`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `OBELISK_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.obelisk` e `~/.config/obelisk` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/obelisk`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Obelisk in `~/.config/obelisk/obelisk.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/obelisk/tui.json`.
...
Aggiungi `obelisk.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Obelisk si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `OBELISK_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OBELISK_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.obelisk` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.obelisk`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`obelisk.ai/config.json`**](https://obelisk.ai/config.json).

La configurazione TUI usa [**`obelisk.ai/tui.json`**](https://obelisk.ai/tui.json).
...
Usa `OBELISK_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `obelisk.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `obelisk serve` e `obelisk web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Obelisk.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `obelisk.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Obelisk tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Obelisk prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/obelisk/agents/` o `.obelisk/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Obelisk fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`obelisk run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/obelisk/commands/` o `.obelisk/commands/`. [Scopri di piu](/docs/commands).
...
Obelisk scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, obelisk **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Obelisk con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.obelisk/plugins/` o `~/.config/obelisk/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Obelisk a usare solo alcuni provider invece di disabilitarli uno per uno.
