---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć obelisk.
---

Wtyczki umożliwiające wyłączenie obelisk poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania obelisk.
...
- `.obelisk/plugins/` - Wtyczki na poziomie projektu
- `~/.config/obelisk/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/obelisk/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/obelisk/obelisk.json`)
2. Konfiguracja projektu (`obelisk.json`)
3. Globalny katalog wtyczek (`~/.config/obelisk/plugins/`)
4. Katalog wtyczek projektu (`.obelisk/plugins/`)
...
obelisk uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z obelisk do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do obelisk.
...
:::note
Jeśli korzystasz z aplikacji komputerowej obelisk, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj obelisk czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do obelisk:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod obelisk. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://obelisk.ai/docs/sdk).
