---
title: الخادم
description: تفاعل مع خادم obelisk عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `obelisk serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل obelisk استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `obelisk.local` |
...
عيّن `OBELISK_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `obelisk`، أو عيّن `OBELISK_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `obelisk serve` و `obelisk web`.
...
عند تشغيل `obelisk` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم obelisk للتفاعل مع obelisk برمجيا.
:::

تتيح هذه البنية لـ obelisk دعم عدة عملاء وتمكّنك من التفاعل مع obelisk برمجيا.

يمكنك تشغيل `obelisk serve` لبدء خادم مستقل. إذا كانت واجهة obelisk في terminal (TUI)
قيد التشغيل، فسيبدأ `obelisk serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Obelisk لـ [IDE](/docs/ide).
...
يعرض خادم obelisk واجهات API التالية.
