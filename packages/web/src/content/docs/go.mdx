...
Obelisk Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Obelisk. You subscribe to Obelisk Go and
...
Obelisk Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Obelisk Go works like any other provider in Obelisk.

1. You sign in to **<a href={console}>Obelisk Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Obelisk Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Obelisk Go.
:::
...
Obelisk Go includes the following limits:
...
uses the format `obelisk-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `obelisk-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Obelisk as well.
