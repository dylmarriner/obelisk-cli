...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@obelisk-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@obelisk-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@obelisk-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@obelisk-ai/tui/context/theme"
import { Global } from "@obelisk-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@obelisk-ai/core/util/flock"
import { Flag } from "@obelisk-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@obelisk-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@obelisk-ai/core/v1/config/plugin"
import { createCommandShim } from "@obelisk-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@obelisk-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".obelisk"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".obelisk", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".obelisk", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.OBELISK_PURE ? [] : pluginOrigins
    if (Flag.OBELISK_PURE && pluginOrigins.length) {
    }
