...
Trim `packages/obelisk/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/obelisk/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/obelisk`.
- `[x]` `bun typecheck` from `packages/obelisk`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/obelisk/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/obelisk`.
- `[x]` `bun dev generate > /tmp/obelisk-openapi.json` from `packages/obelisk`.
...
- Edit `packages/obelisk/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/obelisk-openapi.json` from `packages/obelisk`.
...
- Check source schemas in files like `packages/obelisk/src/session/schema.ts`, `packages/obelisk/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/obelisk/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/obelisk/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
