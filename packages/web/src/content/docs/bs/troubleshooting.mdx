...
Da biste otklonili probleme s Obelisk, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\obelisk\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `obelisk --log-level DEBUG`.

---

## Pohrana

Obelisk pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\obelisk`
...
Obelisk Desktop pokreće lokalni Obelisk server (`obelisk-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Obelisk` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/obelisk/obelisk.jsonc` (ili `~/.config/obelisk/obelisk.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
Obelisk također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\obelisk\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Obelisk mogao ponovo izgraditi.

1. Potpuno zatvorite Obelisk Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/obelisk`
- **Linux**: obrišite `~/.cache/obelisk` (ili pokrenite `rm -rf ~/.cache/obelisk`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\obelisk`

3. Ponovo pokrenite Obelisk Desktop.

---

### Rješavanje problema sa vezom na serveru

Obelisk Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `obelisk.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OBELISK_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OBELISK_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Obelisk Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Obelisk karakteristikama.

---

### Obavještenja se ne prikazuju

Obelisk Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Obelisk u postavkama vašeg OS-a, i
...
1. Zatvorite Obelisk Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Obelisk Desktop aplikacije):

- `obelisk.settings.dat` (URL zadanog servera za desktop)
- `obelisk.global.dat` i `obelisk.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `obelisk models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\obelisk`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Obelisk dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\obelisk`

2. Ponovo pokrenite Obelisk da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Obelisk da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Obelisk će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
