...
Aby debugować problemy z obelisk, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\obelisk\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `obelisk --log-level DEBUG`.

---

## Przechowywanie danych

obelisk przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\obelisk`
...
obelisk Desktop uruchamia w tle lokalny serwer obelisk (sidecar `obelisk-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Obelisk` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/obelisk/obelisk.jsonc` (lub `~/.config/obelisk/obelisk.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
obelisk może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\obelisk\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby obelisk mógł ją odbudować.

1. Całkowicie zamknij obelisk Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/obelisk`
- **Linux**: usuń `~/.cache/obelisk` (lub uruchom `rm -rf ~/.cache/obelisk`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\obelisk`

3. Uruchom ponownie obelisk Desktop.

---

### Napraw problemy z połączeniem z serwerem

obelisk Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `obelisk.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OBELISK_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OBELISK_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows obelisk Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami obelisk.

---

### Brak powiadomień

obelisk Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla obelisk w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie obelisk Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji obelisk Desktop):

- `obelisk.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `obelisk.global.dat` i `obelisk.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `obelisk upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `obelisk models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\obelisk`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. obelisk dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\obelisk`

2. Uruchom ponownie kod obelisk, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to obelisk do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
obelisk wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
