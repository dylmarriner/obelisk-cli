...
أنشئ الملف `.obelisk/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Obelisk أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Obelisk:
...
- على مستوى النظام: `~/.config/obelisk/commands/`
- لكل مشروع: `.obelisk/commands/`
...
يتضمن obelisk عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
