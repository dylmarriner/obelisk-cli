...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Obelisk dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Obelisk.
...
:::note
Ovaj alat je dostupan samo kada je `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (ili `OBELISK_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Obelisk provajdera ili kada je varijabla `OBELISK_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Obelisk:
