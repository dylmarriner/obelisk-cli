---
title: Policies
description: Control which configured resources Obelisk may use.
---

Policies control whether Obelisk may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `obelisk.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Obelisk may use a resource such as an LLM provider.
...
Obelisk currently supports one policy action:
