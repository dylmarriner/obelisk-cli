...
Ajan becerileri, obelisk'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
obelisk su konumlari tarar:

- Proje konfigurasyonu: `.obelisk/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/obelisk/skills/<name>/SKILL.md`
...
Proje yerel yollarinda obelisk, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.obelisk/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.obelisk/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
obelisk, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`obelisk.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`obelisk.json` icinde):
