...
Obelisk Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Obelisk-u. Pretplatite se na Obelisk Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Obelisk.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Obelisk-om.
:::
...
Obelisk Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Obelisk Go radi kao bilo koji drugi provajder u Obelisk-u.

1. Prijavite se na **<a href={console}>Obelisk Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Obelisk Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Obelisk Go.
:::
...
Obelisk Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Obelisk konfiguraciji
koristi format `obelisk-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`obelisk-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo Obelisk Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Obelisk koristite i bilo kojeg drugog provajdera.
