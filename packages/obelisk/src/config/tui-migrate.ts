...
const TUI_SCHEMA_URL = "https://obelisk.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from obelisk.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const obelisk = await obeliskFiles(input)
  for (const file of obelisk) {
...
async function obeliskFiles(input: { directories: string[]; cwd: string }) {
  ...
  if (Flag.OBELISK_CONFIG) files.push(Flag.OBELISK_CONFIG)
