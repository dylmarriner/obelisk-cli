...
In Obelisk esistono due tipi di agenti: agenti primari e subagenti.
...
Obelisk include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Obelisk include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Obelisk include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Obelisk, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `obelisk.json`:
...
- Globale: `~/.config/obelisk/agents/`
- Per progetto: `.obelisk/agents/`
...
Se non specifichi una temperature, Obelisk usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Obelisk sia per la config specifica del progetto.
...
L'ID modello nella configurazione Obelisk usa il formato `provider/model-id`. Per esempio, se stai usando [Obelisk Zen](/docs/zen), useresti `obelisk/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `obelisk models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/obelisk).
:::
