...
_Model Bağlam Protokolü_ veya MCP kullanarak obelisk'a harici araçlar ekleyebilirsiniz. obelisk hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [obelisk Config](https://obelisk.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/obelisk` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
obelisk, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde obelisk şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa obelisk, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `obelisk mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından obelisk, belirteçleri `~/.local/share/obelisk/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra obelisk'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi obelisk yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve obelisk'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
