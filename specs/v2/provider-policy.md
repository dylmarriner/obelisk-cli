...
Both `action` and `resource` use obelisk's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.obelisk` files is intentionally deferred until `.obelisk` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether obelisk permits an operation through its managed execution paths.
