---
title: Servidor
description: Interaja com o servidor obelisk via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `obelisk serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente obelisk pode usar.
...
```bash
obelisk serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `obelisk.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
obelisk serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OBELISK_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `obelisk`, ou defina `OBELISK_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `obelisk serve` quanto ao `obelisk web`.

```bash
OBELISK_SERVER_PASSWORD=your-password obelisk serve
```
...
Quando você executa `obelisk`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor obelisk para interagir com o obelisk programaticamente.
:::

Essa arquitetura permite que o obelisk suporte múltiplos clientes e permite que você interaja com o obelisk programaticamente.

Você pode executar `obelisk serve` para iniciar um servidor autônomo. Se você tiver o TUI do obelisk em execução, `obelisk serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do obelisk [IDE](/docs/ide).
...
O servidor obelisk expõe as seguintes APIs.
