---
title: Konfiguracija
description: Korištenje Obelisk JSON konfiguracije.
---

Možete konfigurirati Obelisk koristeći JSON konfiguracijski fajl.

---

## Format

Obelisk podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/obelisk`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/obelisk/obelisk.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OBELISK_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`obelisk.json` u projektu) - postavke specifične za projekat
5. **`.obelisk` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OBELISK_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.obelisk` i `~/.config/obelisk` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/obelisk` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Obelisk konfiguraciju u `~/.config/obelisk/obelisk.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/obelisk/tui.json`.
...
Dodajte `obelisk.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Obelisk pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OBELISK_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OBELISK_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.obelisk` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.obelisk`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`obelisk.ai/config.json`**](https://obelisk.ai/config.json).

TUI konfiguracija koristi [**`obelisk.ai/tui.json`**](https://obelisk.ai/tui.json).
...
Koristite `OBELISK_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `obelisk.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `obelisk serve` i `obelisk web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Obelisk server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `obelisk.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Obelisk konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Obelisk pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/obelisk/agents/` ili `.obelisk/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Obelisk će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`obelisk run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/obelisk/commands/` ili `.obelisk/commands/`. [Saznajte više ovdje](/docs/commands).
...
Obelisk će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Obelisk **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Obelisk sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.obelisk/plugins/` ili `~/.config/obelisk/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Obelisk da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
