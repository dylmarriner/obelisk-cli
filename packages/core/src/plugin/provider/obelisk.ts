...
import type { IntegrationOAuthMethodRegistration } from "@obelisk-ai/plugin/v2/effect/integration"
import { define } from "@obelisk-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@obelisk-ai/sdk/v2/types"
...
const defaultServer = "https://console.obelisk.ai"
const clientID = "obelisk-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("obelisk")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Obelisk provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("obelisk", (integration) => {
        integration.name = "Obelisk"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "obelisk", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("obelisk")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("obelisk")
      ...
      const item = catalog.provider.get(ProviderV2.ID.obelisk)
      if (!item) return
      const hasKey = Boolean(process.env.OBELISK_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("obelisk")),
