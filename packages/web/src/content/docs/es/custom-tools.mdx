...
description: Cree herramientas que LLM pueda llamar en obelisk.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de obelisk como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.obelisk/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/obelisk/tools/`.
...
```ts title=".obelisk/tools/database.ts" {1}
...
```ts title=".obelisk/tools/math.ts"
import { tool } from "@obelisk-ai/plugin"
...
```ts title=".obelisk/tools/bash.ts"
import { tool } from "@obelisk-ai/plugin"
...
```ts title=".obelisk/tools/project.ts" {8}
import { tool } from "@obelisk-ai/plugin"
...
```python title=".obelisk/tools/add.py"
import sys
...
```ts title=".obelisk/tools/python-add.ts" {10}
import { tool } from "@obelisk-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".obelisk/tools/add.py")
