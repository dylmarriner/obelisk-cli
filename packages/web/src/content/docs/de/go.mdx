...
Obelisk Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Obelisk. Du abonnierst Obelisk Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Obelisk zu verwenden.
...
Obelisk Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Obelisk Go funktioniert wie jeder andere Provider in Obelisk.

1. Du meldest dich bei **<a href={console}>Obelisk Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Obelisk Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Obelisk Go abonnieren.
:::
...
Obelisk Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Obelisk Config verwendet das Format `obelisk-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `obelisk-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Obelisk Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Obelisk zu nutzen.
