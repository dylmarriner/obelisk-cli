...
For at fejlfinde problemer med Obelisk, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\obelisk\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `obelisk --log-level DEBUG`.

---

## Lagring

obelisk gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\obelisk`
...
Obelisk Desktop kører en lokal Obelisk-server (`obelisk-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Obelisk`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
Obelisk kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\obelisk\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Obelisk kan genopbygge den.

1. Afslut Obelisk Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/obelisk`
- **Linux**: slet `~/.cache/obelisk` (eller kør `rm -rf ~/.cache/obelisk`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\obelisk`

3. Start Obelisk Desktop på ny.

---

### Løs problemer med serverforbindelse

Obelisk Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `obelisk.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OBELISK_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OBELISK_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Obelisk Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Obelisk.

---

### Meddelelser vises ikke

Obelisk Desktop viser kun systemvarsler når:

- varsler er aktiveret for Obelisk i dine OS-indstillinger, og
...
1. Afslut Obelisk Desktop.
2. Find og slet disse filer (de findes i Obelisk Desktop-appens datamappe):

- `obelisk.settings.dat` (skrivebordsstandardserver URL)
- `obelisk.global.dat` og `obelisk.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Obelisk:
...
For at finde ud af hvilke modeller du har adgang til, kør `obelisk models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\obelisk`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. obelisk installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\obelisk`

2. Start obelisk på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge obelisk til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
obelisk vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
