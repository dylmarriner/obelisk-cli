...
Med Obelisk kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Obelisk vårt eget `obelisk`-tema.
...
Obelisk støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/obelisk/themes/*.json` eller `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.obelisk/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.obelisk/themes/*.json`
