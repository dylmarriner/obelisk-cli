...
import { GoogleVertexPlugin } from "@obelisk-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.obelisk, (provider) => {
          provider.api = {
            ...
            url: "https://obelisk.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.obelisk))
