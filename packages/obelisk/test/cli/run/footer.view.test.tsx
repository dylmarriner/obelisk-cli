...
import type { QuestionRequest } from "@obelisk-ai/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@obelisk-ai/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "obelisk", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "obelisk", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("obelisk")
    expect(frame).toContain("GPT-5")
