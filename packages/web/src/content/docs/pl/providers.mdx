---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

obelisk używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji obelisk.
...
w `~/.local/share/obelisk/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim obelisk
config.
...
Obelisk Zen to lista modeli dostarczonych przez zespół obelisk, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z obelisk. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Obelisk Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Obelisk Zen` i przejdź do [obelisk.ai/auth](https://obelisk.ai/zen).
...
Działa jak każdy inny dostawca w obelisk i jest całkowicie opcjonalny w użyciu.

---

## Obelisk Go

Obelisk Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół obelisk, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z obelisk.

1. Uruchom polecenie `/connect` w TUI, wybierz `Obelisk Go` i przejdź do [obelisk.ai/auth](https://obelisk.ai/zen).
...
Aby używać Amazon Bedrock z obelisk:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania obelisk:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `obelisk.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w obelisk nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować obelisk do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod obelisk działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji obelisk.
...
obelisk używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować obelisk
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`obelisk.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem obelisk:
...
Aby używać Google Vertex AI z obelisk:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania obelisk.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą obelisk, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session), aby automatycznie rejestrować każdą konwersację obelisk jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja obelisk będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować obelisk tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować obelisk tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować obelisk tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem obelisk. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/obelisk).
:::
...
Aby korzystać z Ollama Cloud z obelisk:
...
3. Skopiuj klucz API do użycia w obelisk.
...
6. **Ważne**: Przed użyciem modeli chmurowych w obelisk musisz pobrać informacje o modelu lokalnie:
...
Obelisk Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół obelisk. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Obelisk Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Obelisk Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API obelisk.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji obelisk.
...
5. Możesz je także dostosować za pomocą konfiguracji obelisk. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z obelisk:
...
Możesz także dostosować modele za pomocą konfiguracji obelisk. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z obelisk. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `obelisk.json` w katalogu projektu:
...
Pola `limit` pozwalają obelisk zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `obelisk auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację obelisk i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji obelisk.
