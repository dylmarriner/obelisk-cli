...
const model = "obelisk/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/obelisk")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with obelisk...`)
  ...
  try {
    await $`obelisk run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  obelisk failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with obelisk")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with obelisk...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/obelisk`.",
