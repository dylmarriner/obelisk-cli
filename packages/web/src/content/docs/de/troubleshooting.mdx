...
Wenn Obelisk Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\obelisk\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `obelisk --log-level DEBUG`.

---

## Speicher

obelisk speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/obelisk/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\obelisk`
...
Obelisk Desktop startet im Hintergrund einen lokalen Obelisk-Server (`obelisk-cli`-Sidecar).
...
- Nur macOS: `Obelisk`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/obelisk/obelisk.jsonc` (oder `~/.config/obelisk/obelisk.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/obelisk/obelisk.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\obelisk\obelisk.jsonc`
...
Obelisk kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/obelisk/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\obelisk\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Obelisk ihn neu aufbauen kann.

1. Beende Obelisk Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/obelisk`
- **Linux**: loesche `~/.cache/obelisk` (oder fuehre aus: `rm -rf ~/.cache/obelisk`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\obelisk`

3. Starte Obelisk Desktop neu.

---

### Server-Verbindungsprobleme beheben

Obelisk Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `obelisk.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OBELISK_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OBELISK_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Obelisk Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Obelisk-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Obelisk Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Obelisk in den OS-Einstellungen aktiviert sind, und
...
1. Beende Obelisk Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Obelisk Desktop):

- `obelisk.settings.dat` (desktop default server URL)
- `obelisk.global.dat` und `obelisk.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Obelisk hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `obelisk models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\obelisk`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. obelisk installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\obelisk`

2. Starte obelisk neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt obelisk, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
obelisk erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
