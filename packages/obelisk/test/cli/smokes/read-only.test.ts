...
describe("obelisk read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OBELISK_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["mcp", "list"])
        obelisk.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["providers", "list"])
        obelisk.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["models"])
        obelisk.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["agent", "list"])
        obelisk.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OBELISK_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["session", "list"])
        obelisk.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["stats"])
        obelisk.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OBELISK_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ obelisk }) =>
      Effect.gen(function* () {
        const r = yield* obelisk.spawn(["db", "path"])
        obelisk.expectExit(r, 0, "db path")
