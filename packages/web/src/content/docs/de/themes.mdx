...
Mit Obelisk kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Obelisk das `obelisk`-Theme.
...
Obelisk liefert mehrere eingebaute Themes mit.
...
- Obelisk optisch ans Terminal anpassen wollen
...
Obelisk unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/obelisk/themes/*.json` oder `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.obelisk/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.obelisk/themes/*.json`
