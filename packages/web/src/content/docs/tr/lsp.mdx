...
description: Obelisk, LSP sunucularınızla bütünleşir.
---

Obelisk, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Obelisk, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`OBELISK_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve obelisk bir dosyayı açtığında:
...
LSP sunucularını, obelisk yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
