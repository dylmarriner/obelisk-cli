...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.obelisk.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `obelisk.settings.dat` is special: it maps to the `obelisk.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"obelisk.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // obelisk.settings.dat → the electron settings store ("obelisk.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "obelisk.global.dat").
  const storeName = filename === "obelisk.settings.dat" ? "obelisk.settings" : filename
