...
Obelisk zawiera listę skrótów klawiszowych, które można dostosować za pomocą `tui.json`.
...
obelisk używa klucza `leader` dla głównych skrótów klawiszowych. Rozwiązanie problemu konfliktów w terminalu.
...
Pole sprawdzania aplikacji komputerowej obelisk obsługuje popularne skróty w stylu Readline/Emacs do edycji tekstu. Są one dostępne i obecnie nie można ich skonfigurować za pomocą `obelisk.json`.
