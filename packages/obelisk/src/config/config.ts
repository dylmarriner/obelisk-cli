...
import { serviceUse } from "@obelisk-ai/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@obelisk-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@obelisk-ai/core/v1/config/console-state"
import { FSUtil } from "@obelisk-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@obelisk-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@obelisk-ai/core/v1/config/config"
import { RemoteAuthError } from "@obelisk-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@obelisk-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@obelisk/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["obelisk.jsonc", "obelisk.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://obelisk.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OBELISK_CONFIG && !Flag.OBELISK_CONFIG_DIR && !Flag.OBELISK_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://obelisk.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "obelisk.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "obelisk.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://obelisk.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OBELISK_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/obelisk`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://obelisk.ai/config.json"
        ...
        if (Flag.OBELISK_CONFIG) {
          yield* merge(Flag.OBELISK_CONFIG, yield* loadFile(Flag.OBELISK_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.OBELISK_CONFIG })
        }
        ...
        if (Flag.OBELISK_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OBELISK_CONFIG_DIR", { path: Flag.OBELISK_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".obelisk") || dir === Flag.OBELISK_CONFIG_DIR) {
            for (const file of ["obelisk.json", "obelisk.jsonc"]) {
          ...
          // Auto-discovered plugins under `.obelisk/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OBELISK_CONFIG_CONTENT) {
          const source = "OBELISK_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OBELISK_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from OBELISK_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OBELISK_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OBELISK_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OBELISK_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OBELISK_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("OBELISK_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OBELISK_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OBELISK_DISABLE_PRUNE) {
