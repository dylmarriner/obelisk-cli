...
import { isRecord } from "@obelisk-ai/tui/util/record"
...
import { TuiKeybind } from "@obelisk-ai/tui/config/keybind"
...
import { TuiConfig } from "@obelisk-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@obelisk/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old obelisk.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.obelisk`
  // folders between cwd and home, and OBELISK_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OBELISK_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OBELISK_TUI_CONFIG override, if set.
  if (Flag.OBELISK_TUI_CONFIG) {
    const configFile = Flag.OBELISK_TUI_CONFIG
  ...
  // 4. `.obelisk` directories (and OBELISK_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".obelisk") || dir === Flag.OBELISK_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".obelisk") && dir !== Flag.OBELISK_CONFIG_DIR) continue
