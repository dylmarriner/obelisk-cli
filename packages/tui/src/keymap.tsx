...
export const OBELISK_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OBELISK_MODE_KEY = "obelisk.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OBELISK_MODE_KEY, OBELISK_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OBELISK_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OBELISK_MODE_KEY, stack.at(-1)?.mode ?? OBELISK_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OBELISK_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OBELISK_MODE_KEY, undefined)
