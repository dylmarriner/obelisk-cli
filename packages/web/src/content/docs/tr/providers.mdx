---
title: Sağlayıcılar
description: obelisk'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

obelisk, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı obelisk yapılandırmanızda yapılandırın.
...
`~/.local/share/obelisk/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları obelisk'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Obelisk Zen, obelisk ekibi tarafından sağlanan modellerin bir listesidir.
obelisk ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Obelisk Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, obelisk'u seçin ve [obelisk.ai/auth](https://obelisk.ai/auth)'ye gidin.
...
obelisk'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Obelisk Go

Obelisk Go, obelisk ile iyi çalıştığı test edilmiş ve doğrulanmış, obelisk ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Obelisk Go`'yu seçin ve [obelisk.ai/auth](https://obelisk.ai/zen) adresine gidin.
...
Amazon Bedrock'u obelisk ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   obelisk'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `obelisk.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi obelisk'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
obelisk'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   obelisk'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
obelisk, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. obelisk'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`obelisk.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth) ana sayfasında.

##### Yapılandırma

`obelisk.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi obelisk'la kullanmak için:
...
Google Vertex AI'yi obelisk ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları obelisk çalıştırılırken ayarlayın.
...
Helicone'un obelisk aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her obelisk görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her obelisk görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
obelisk'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla obelisk'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla obelisk'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini obelisk için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/obelisk)'a bakın.
:::
...
Ollama Cloud'u obelisk ile kullanmak için:
...
3. obelisk'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: obelisk'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Obelisk Zen, obelisk ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Obelisk Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Obelisk Zen**'i arayın.

   ```txt
   /connect
   ```

3. obelisk API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   obelisk yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca obelisk yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri obelisk yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı obelisk ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `obelisk.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları obelisk'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `obelisk auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için obelisk yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, obelisk yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
