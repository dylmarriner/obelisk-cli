---
title: Sunucu
description: obelisk sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`obelisk serve` komutu, obelisk istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `obelisk.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OBELISK_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `obelisk` değeridir; değiştirmek isterseniz `OBELISK_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `obelisk serve` hem de `obelisk web` için geçerlidir.
...
`obelisk` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
obelisk ile programatik etkileşim için obelisk sunucusunu kullanın.
:::

Bu mimari, obelisk'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`obelisk serve` ile bağımsız bir sunucu başlatabilirsiniz. obelisk TUI açıksa, `obelisk serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum obelisk [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
obelisk sunucusu aşağıdaki API'leri sunar.
