...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "obelisk.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "obelisk.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "obelisk.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "obelisk.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["obelisk.json", "obelisk.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "obelisk.json"),
        path.join(project, "obelisk.jsonc"),
      ])
    })
  })
