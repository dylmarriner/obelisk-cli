// Subprocess test harness for the obelisk CLI. Spawns the real binary against
...
//
// Configuration flows through obelisk's built-in test affordances:
//   - OBELISK_CONFIG_CONTENT      : provider config inline, no files to find
//   - OBELISK_TEST_HOME           : pins os.homedir() → tmpdir
//   - OBELISK_DISABLE_PROJECT_CONFIG : skip walking up for obelisk.json
//   - OBELISK_PURE                : skip external plugin discovery + install
//   - OBELISK_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `obelisk.run` is fully wired. The shape supports adding more
// builders (`obelisk.serve(opts)`, `obelisk.acp(opts)`, `obelisk.auth(...)`)
...
import { AppProcess } from "@obelisk-ai/core/process"
...
const obeliskRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(obeliskRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OBELISK_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OBELISK_CONFIG_CONTENT: configJson,
    OBELISK_DISABLE_PROJECT_CONFIG: "1",
    OBELISK_PURE: "1",
    OBELISK_DISABLE_AUTOUPDATE: "1",
    OBELISK_DISABLE_AUTOCOMPACT: "1",
    OBELISK_DISABLE_MODELS_FETCH: "1",
    OBELISK_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `obelisk run`. New flags should
...
// `obelisk serve` is a long-lived process — it never exits on its own.
...
// `obelisk acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `obelisk serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `obelisk acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("obelisk.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OBELISK_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("obelisk.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("obelisk.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "obelisk server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `obelisk serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("obelisk.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const obelisk: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, obelisk })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `obelisk.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "obelisk") {
...
// every call site. Use this for any test that needs the obelisk CLI fixture.
//
...
// resources (e.g. `obelisk.serve`) without an extra `Effect.scoped` wrapper —
