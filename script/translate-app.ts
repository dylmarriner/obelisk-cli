...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "obelisk/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".obelisk/glossary/zh-cn.md"
  if (locale === "zht") return ".obelisk/glossary/zh-tw.md"
  return `.obelisk/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Obelisk did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Obelisk returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Obelisk session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Obelisk runs for 'all' (default: 4)
      --model <provider/id>  Obelisk model (default: obelisk/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Obelisk
...
`)
  ...
  if (failed.length) console.error(`\nObelisk failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.OBELISK_DISABLE_PROJECT_CONFIG = "1"
  env.OBELISK_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["obelisk", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OBELISK_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["obelisk", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.OBELISK_CONFIG
  delete env.OBELISK_CONFIG_DIR
  delete env.OBELISK_CONFIG_CONTENT
  delete env.OBELISK_PERMISSION
  delete env.OBELISK_AUTO_SHARE
  return env
}
