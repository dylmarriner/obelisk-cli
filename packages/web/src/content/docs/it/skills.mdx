...
Le skill degli agenti permettono a Obelisk di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Obelisk cerca in queste posizioni:

- Config di progetto: `.obelisk/skills/<name>/SKILL.md`
- Config globale: `~/.config/obelisk/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Obelisk risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.obelisk/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.obelisk/skills/git-release/SKILL.md` cosi':
...
Obelisk elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `obelisk.json`:
...
**Per agenti integrati** (in `obelisk.json`):
