...
import { and, Database, eq, isNull, lt, or, sql } from "@obelisk-ai/console-core/drizzle/index.js"
import { KeyTable } from "@obelisk-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@obelisk-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@obelisk-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@obelisk-ai/console-core/util/date.js"
import { Identifier } from "@obelisk-ai/console-core/identifier.js"
import { Billing } from "@obelisk-ai/console-core/billing.js"
import { Actor } from "@obelisk-ai/console-core/actor.js"
import { WorkspaceTable } from "@obelisk-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@obelisk-ai/console-core/model.js"
import { Subscription } from "@obelisk-ai/console-core/subscription.js"
import { BlackData } from "@obelisk-ai/console-core/black.js"
import { UserTable } from "@obelisk-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@obelisk-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@obelisk-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@obelisk-ai/console-core/lite.js"
import { Resource } from "@obelisk-ai/console-resource"
...
import { Workspace } from "@obelisk-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-obelisk-session") ?? ""
    const requestId = input.request.headers.get("x-obelisk-request") ?? ""
    const ocClient = input.request.headers.get("x-obelisk-client") ?? ""
    const projectId = input.request.headers.get("x-obelisk-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://obelisk.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-obelisk-request")
            headers.delete("x-obelisk-session")
            headers.delete("x-obelisk-project")
            headers.delete("x-obelisk-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-obelisk-endpoint-id")
        const resEndpointModelId = res.headers.get("x-obelisk-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://obelisk.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://obelisk.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://obelisk.ai/workspace/${authInfo.workspaceID}/members`
