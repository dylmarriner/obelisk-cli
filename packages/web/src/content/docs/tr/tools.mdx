...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. obelisk yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
obelisk içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (veya `OBELISK_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca obelisk provider kullanılırken veya `OBELISK_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

obelisk başlatırken etkinleştirmek için:
