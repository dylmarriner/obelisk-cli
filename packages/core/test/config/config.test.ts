...
import { ConfigProvider } from "@obelisk-ai/core/config/provider"
...
import { ConfigMigrateV1 } from "@obelisk-ai/core/v1/config/migrate"
...
import { Policy } from "@obelisk-ai/core/policy"
import { Project } from "@obelisk-ai/core/project"
import { AbsolutePath } from "@obelisk-ai/core/schema"
...
describe("Config", () => {
  ...
  it.live("loads obelisk JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "obelisk.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "obelisk.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "obelisk.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "obelisk.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "obelisk.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "obelisk.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .obelisk configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".obelisk"), { recursive: true })
            await fs.mkdir(path.join(directory, ".obelisk"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "obelisk.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "obelisk.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "obelisk.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "obelisk.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "obelisk.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".obelisk", "obelisk.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".obelisk", "obelisk.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".obelisk")),
              AbsolutePath.make(path.join(directory, ".obelisk")),
            ])
