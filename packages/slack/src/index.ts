...
import { createOpencode, type ToolPart } from "@obelisk-ai/sdk"
...
console.log("🚀 Starting obelisk server...")
const obelisk = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await obelisk.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new obelisk session...")
    const { client, server } = obelisk
    ...
    console.log("✅ Created obelisk session:", createResult.data.id)
  ...
  console.log("📝 Sending to obelisk:", message.text)
