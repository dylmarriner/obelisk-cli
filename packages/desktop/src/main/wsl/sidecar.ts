...
export async function spawnWslSidecar(
  ...
  const obelisk = await resolveWslOpencode(distro)
  if (!obelisk) throw new Error(`Obelisk is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "obelisk"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OBELISK_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OBELISK_CLIENT=desktop",
    `export OBELISK_SERVER_USERNAME=${shellEscape(username)}`,
    `export OBELISK_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(obelisk)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
