---
title: Plugin
description: Scrivi plugin per estendere Obelisk.
---

I plugin ti permettono di estendere Obelisk agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Obelisk.
...
- `.obelisk/plugins/` - plugin a livello progetto
- `~/.config/obelisk/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/obelisk/node_modules/`.
...
1. Config globale (`~/.config/obelisk/obelisk.json`)
2. Config di progetto (`obelisk.json`)
3. Directory plugin globale (`~/.config/obelisk/plugins/`)
4. Directory plugin di progetto (`.obelisk/plugins/`)
...
Obelisk esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di obelisk per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere obelisk.
...
:::note
Se usi l'app desktop di Obelisk, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a obelisk di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a obelisk:
...
L'helper `tool` crea uno strumento personalizzato che obelisk può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in obelisk insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://obelisk.ai/docs/sdk) per i dettagli.
