---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

Obelisk bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Obelisk-konfigurasjonen.
...
i `~/.local/share/obelisk/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Obelisk
konfig.
...
Obelisk Zen er en liste over modeller levert av Obelisk-teamet som har vært
testet og verifisert for å fungere godt med Obelisk. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Obelisk Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Obelisk Zen` og gå til [obelisk.ai/auth](https://obelisk.ai/zen).
...
Det fungerer som alle andre leverandører i Obelisk og er helt valgfritt å bruke.

---

## Obelisk Go

Obelisk Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Obelisk-teamet som har vært
testet og verifisert for å fungere godt med Obelisk.

1. Kjør kommandoen `/connect` i TUI, velg `Obelisk Go`, og gå til [obelisk.ai/auth](https://obelisk.ai/zen).
...
Slik bruker du Amazon Bedrock med Obelisk:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Obelisk:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `obelisk.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i obelisk støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere obelisk til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Obelisk skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Obelisk-konfigurasjonen.
...
for å lagre token i Obelisk auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Obelisk bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Obelisk
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`obelisk.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `obelisk.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Obelisk:
...
Slik bruker du Google Vertex AI med Obelisk:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Obelisk.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Obelisk, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session) plugin for automatisk å logge hver Obelisk-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Obelisk-samtale oppført som en separat økt.
...
Du kan konfigurere Obelisk for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Obelisk for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Obelisk for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Obelisk. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/obelisk) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Obelisk:
...
3. Kopier API-nøkkelen for bruk i Obelisk.
...
6. **Viktig**: Før du bruker skymodeller i Obelisk, må du hente modellinformasjonen lokalt:
...
Obelisk Zen er en liste over testede og verifiserte modeller levert av Obelisk-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Obelisk Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Obelisk Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Obelisk API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Obelisk-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Obelisk-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Obelisk:
...
Du kan også tilpasse modeller gjennom Obelisk-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Obelisk. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `obelisk.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Obelisk forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `obelisk auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Obelisk-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Obelisk-konfigurasjonen.
