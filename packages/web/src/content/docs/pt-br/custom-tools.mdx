...
description: Crie ferramentas que o LLM pode chamar no obelisk.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do obelisk, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.obelisk/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/obelisk/tools/`.
