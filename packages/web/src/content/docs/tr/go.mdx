...
Obelisk Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Obelisk'daki diğer sağlayıcılar gibi çalışır. Obelisk Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Obelisk'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Obelisk Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Obelisk Go, Obelisk'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Obelisk Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Obelisk Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Obelisk Go'ya abone olabilir.
:::
...
Obelisk Go aşağıdaki limitleri içerir:
...
Obelisk yapılandırmanızdaki [model id](/docs/config/#models) formatı `obelisk-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `obelisk-go/kimi-k2.7-code` kullanmalısınız.
...
Obelisk Go'yu şu amaçlarla oluşturduk:
...
4. Obelisk ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
