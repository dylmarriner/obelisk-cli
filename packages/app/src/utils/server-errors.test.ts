...
import type { SessionNotFoundError } from "@obelisk-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em obelisk.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
