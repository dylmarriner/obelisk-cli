...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "Obelisk update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Obelisk", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.obelisk/bin/obelisk",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, obeliskChecks: {} })
})
...
test("ignores stale background Obelisk checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().obeliskChecks).toEqual({})
})

test("ignores stale startup Obelisk checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Obelisk", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      obelisk.push(distro)
      return "/home/me/.obelisk/bin/obelisk"
    },
  })
  ...
  expect(obelisk).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(obelisk).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().obeliskChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Obelisk in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const obelisk: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(obelisk).toEqual(["Debian"])
  expect(Object.keys(controller.getState().obeliskChecks)).toEqual(["Debian"])
})
