---
title: Serwer
description: Interakcja z serwerem obelisk za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `obelisk serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient obelisk.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `obelisk.local` |
...
Ustaw `OBELISK_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `obelisk` lub ustaw `OBELISK_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `obelisk serve`, jak i `obelisk web`.
...
Kiedy uruchomisz `obelisk`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera obelisk do programowej interakcji z obelisk.
:::

Ta architektura umożliwia obsługę wielu klientów przez obelisk i programową interakcję z kodem otwartym.

Możesz uruchomić `obelisk serve`, aby uruchomić samodzielny serwer. Jeśli masz
obelisk TUI działa, `obelisk serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki obelisk [IDE](/docs/ide).
...
Serwer obelisk udostępnia następujące interfejsy API.
