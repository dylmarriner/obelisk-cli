...
تتيح مهارات الوكيل لـ Obelisk اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Obelisk في هذه المواقع:

- إعدادات المشروع: `.obelisk/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/obelisk/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Obelisk صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.obelisk/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/obelisk/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.obelisk/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Obelisk المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `obelisk.json`:
...
**للوكلاء المدمجين** (في `obelisk.json`):
