---
title: المزوّدون
description: استخدام أي موفّر LLM في Obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Obelisk ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Obelisk.
...
في `~/.local/share/obelisk/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Obelisk.
...
Obelisk Zen هي قائمة نماذج يوفّرها فريق Obelisk وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Obelisk. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Obelisk Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر obelisk، ثم انتقل إلى [obelisk.ai/auth](https://obelisk.ai/auth).
...
يعمل مثل أي مزوّد آخر في Obelisk واستخدامه اختياري بالكامل.

---

## Obelisk Go

Obelisk Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Obelisk، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Obelisk.

1. شغّل الأمر `/connect` في TUI، واختر `Obelisk Go`، ثم انتقل إلى [obelisk.ai/auth](https://obelisk.ai/zen).
...
لاستخدام Amazon Bedrock مع Obelisk:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل obelisk:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `obelisk.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Obelisk غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين obelisk لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل obelisk بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات obelisk.
...
تخزين الرمز في مساحة تخزين مصادقة obelisk.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Obelisk نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Obelisk
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`obelisk.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth).

##### التهيئة

خصّص عبر `obelisk.json`:
...
لاستخدام اشتراك GitHub Copilot مع obelisk:
...
لاستخدام Google Vertex AI مع Obelisk:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر obelisk، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session) لتسجيل كل محادثة Obelisk تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Obelisk مدرجة كجلسة مستقلة.
...
يمكنك ضبط obelisk لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط obelisk لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط obelisk لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Obelisk. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/obelisk) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Obelisk:
...
3. انسخ مفتاح API لاستخدامه في Obelisk.
...
6. **مهم**: قبل استخدام نماذج السحابة في Obelisk، يجب سحب معلومات النموذج محليا:
...
Obelisk Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Obelisk. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Obelisk Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Obelisk Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Obelisk.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات obelisk.
...
5. يمكنك أيضا تخصيصها عبر إعدادات obelisk. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات obelisk. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع obelisk. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `obelisk.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Obelisk بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `obelisk auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات obelisk وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات obelisk.
