  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://obelisk.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://obelisk.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://obelisk.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://obelisk.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://obelisk.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://obelisk.example.com"),
      ),
    ).toEqual({
