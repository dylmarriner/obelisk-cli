...
import { Tag } from "@obelisk-ai/ui/v2/badge-v2"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.obeliskChecks[item.config.distro]
          const obeliskAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-obelisk" && wsl.data.job.distro === item.config.distro
          return (
