...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips obelisk-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-obelisk-directory": "/home/user/project",
      ...
      const result = ProxyUtil.headers(req)
      expect(result.get("x-obelisk-directory")).toBeNull()
      expect(result.get("x-obelisk-workspace")).toBeNull()
