...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@obelisk/acme")).toBe("@obelisk/acme")
    expect(Npm.sanitize("@obelisk/acme@1.0.0")).toBe("@obelisk/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/obelisk/acme.git"
    const expected = win ? "acme@git+https_//github.com/obelisk/acme.git" : spec
