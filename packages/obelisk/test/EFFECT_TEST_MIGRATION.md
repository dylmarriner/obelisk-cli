...
it.instance("instance-local behavior", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("obelisk-test-")
  }),
)
...
- `it.effect(...)` — pure Effect behavior with `TestClock` and
  `TestConsole`.
- `it.instance(...)` — service behavior that needs one scoped obelisk
  instance.
...
9. Run the focused test file and `bun typecheck` from `packages/obelisk`.
...
```bash
git grep -n "Effect.runPromise\|ManagedRuntime\|Promise.withResolvers\|Bun.sleep\|withTestInstance" -- packages/obelisk/test
```
