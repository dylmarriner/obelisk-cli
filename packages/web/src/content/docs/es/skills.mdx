...
Las habilidades del agente permiten a Obelisk descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Obelisk busca estas ubicaciones:

- Configuración del proyecto: `.obelisk/skills/<name>/SKILL.md`
- Configuración global: `~/.config/obelisk/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Obelisk sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.obelisk/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.obelisk/skills/git-release/SKILL.md` así:
...
Obelisk enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `obelisk.json`:
...
**Para agentes integrados** (en `obelisk.json`):
