...
Create `.obelisk/commands/test.md`:
...
You can add custom commands through the Obelisk config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Obelisk [config](/docs/config):
...
obelisk includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
