...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Obelisk posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Obelisk.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (lub `OBELISK_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Obelisk lub gdy zmienna środowiskowa `OBELISK_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Obelisk:
