...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-obelisk-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-obelisk-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://obelisk.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-obelisk-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-obelisk-workspace")
