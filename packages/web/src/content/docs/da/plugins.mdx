---
title: Plugins
description: Skriv dine egne plugins for at udvide Obelisk.
---

Plugins giver dig mulighed for at udvide Obelisk ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Obelisks standardadfærd.
...
- `.obelisk/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/obelisk/node_modules/`.
...
1. Global konfiguration (`~/.config/obelisk/obelisk.json`)
2. Projektkonfiguration (`obelisk.json`)
3. Global plugin-mappe (`~/.config/obelisk/plugins/`)
4. Projekt plugin bibliotek (`.obelisk/plugins/`)
...
Obelisk kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En obelisk SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide obelisk.
...
:::note
Hvis du bruger Obelisk desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå obelisk i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til obelisk:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som obelisk kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for obelisk sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://obelisk.ai/docs/sdk) for detaljer.
