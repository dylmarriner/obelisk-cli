...
import { ObeliskHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for obelisk serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ObeliskHttpApi)
...
export const PublicApi = ObeliskHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "obelisk",
    version: "1.0.0",
    description: "obelisk api",
