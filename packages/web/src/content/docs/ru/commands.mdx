...
Создайте `.obelisk/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию obelisk или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем obelisk [config](/docs/config):
...
- Глобальный: `~/.config/obelisk/commands/`
- Для каждого проекта: `.obelisk/commands/`
...
obelisk включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
