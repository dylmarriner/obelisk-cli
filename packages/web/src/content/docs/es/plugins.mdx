---
title: Complementos
description: Escriba sus propios complementos para extender Obelisk.
---

Los complementos le permiten extender Obelisk al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Obelisk.
...
- `.obelisk/plugins/` - Complementos a nivel de proyecto
- `~/.config/obelisk/plugins/` - Complementos globales
...
```json title="obelisk.json"
{
  ...
  "plugin": ["obelisk-helicone-session", "obelisk-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/obelisk/node_modules/`.
...
1. Configuración global (`~/.config/obelisk/obelisk.json`)
2. Configuración del proyecto (`obelisk.json`)
3. Directorio global de complementos (`~/.config/obelisk/plugins/`)
4. Directorio de complementos del proyecto (`.obelisk/plugins/`)
...
```json title=".obelisk/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Obelisk ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".obelisk/plugins/my-plugin.ts"
...
```js title=".obelisk/plugins/example.js"
...
- `client`: Un cliente SDK obelisk para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar obelisk.
...
```js title=".obelisk/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "obelisk"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Obelisk, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que obelisk lea archivos `.env`:

```javascript title=".obelisk/plugins/env-protection.js"
...
```javascript title=".obelisk/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a obelisk:

```ts title=".obelisk/plugins/custom-tools.ts"
import { type Plugin, tool } from "@obelisk-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que obelisk puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para obelisk junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://obelisk.ai/docs/sdk) para obtener más detalles.
...
```ts title=".obelisk/plugins/compaction.ts"
import type { Plugin } from "@obelisk-ai/plugin"
...
```ts title=".obelisk/plugins/custom-compaction.ts"
import type { Plugin } from "@obelisk-ai/plugin"
