...
يمكنك إضافة أدوات خارجية إلى Obelisk باستخدام _Model Context Protocol_، أو MCP. يدعم Obelisk خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Obelisk](https://obelisk.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/obelisk` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Obelisk تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Obelisk بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Obelisk المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `obelisk mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Obelisk الرموز بشكل آمن في `~/.local/share/obelisk/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Obelisk إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Obelisk مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Obelisk بحساب Sentry الخاص بك.
