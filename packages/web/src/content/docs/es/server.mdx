---
title: Servidor
description: Interactuar con el servidor obelisk a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `obelisk serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente obelisk puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `obelisk.local` |
...
Configure `OBELISK_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `obelisk`, o configure `OBELISK_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `obelisk serve` como a `obelisk web`.
...
Cuando ejecuta `obelisk`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor obelisk para interactuar con obelisk mediante programación.
:::

Esta arquitectura permite que obelisk admita múltiples clientes y le permite interactuar con obelisk mediante programación.

Puede ejecutar `obelisk serve` para iniciar un servidor independiente. Si tienes el
obelisk TUI ejecutándose, `obelisk serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Obelisk [IDE](/docs/ide).
...
El servidor obelisk expone las siguientes API.
