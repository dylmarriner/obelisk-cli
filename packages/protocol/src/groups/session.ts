...
import { PromptInput } from "@obelisk-ai/schema/prompt-input"
import { Session } from "@obelisk-ai/schema/session"
import { Project } from "@obelisk-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@obelisk-ai/schema/schema"
...
import { Revert } from "@obelisk-ai/schema/revert"
import { SessionEvent } from "@obelisk-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Obelisk process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Obelisk process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
