...
Las herramientas permiten que LLM realice acciones en su código base. Obelisk viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Obelisk.
...
:::note
Esta herramienta solo está disponible cuando `OBELISK_EXPERIMENTAL_LSP_TOOL=true` (o `OBELISK_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Obelisk o cuando la variable de entorno `OBELISK_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Obelisk:

```bash
OBELISK_ENABLE_EXA=1 obelisk
```
