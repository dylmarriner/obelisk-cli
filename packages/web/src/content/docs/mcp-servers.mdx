...
You can add external tools to Obelisk using the _Model Context Protocol_, or MCP. Obelisk supports both local and remote servers.
...
You can define MCP servers in your [Obelisk Config](https://obelisk.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/obelisk` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Obelisk automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Obelisk will:
...
If the server requires authentication, Obelisk will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `obelisk mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Obelisk will store the tokens securely in `~/.local/share/obelisk/mcp-auth.json`.
...
Your MCPs are available as tools in Obelisk, alongside built-in tools. So you can manage them through the Obelisk config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Obelisk to your Sentry account.
