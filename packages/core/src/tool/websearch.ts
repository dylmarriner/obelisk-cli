...
export class ConfigService extends Context.Service<ConfigService, Config>()("@obelisk/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.OBELISK_WEBSEARCH_PROVIDER === "exa" || process.env.OBELISK_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.OBELISK_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("OBELISK_EXPERIMENTAL") || truthy("OBELISK_ENABLE_EXA") || truthy("OBELISK_EXPERIMENTAL_EXA"),
    enableParallel: truthy("OBELISK_ENABLE_PARALLEL") || truthy("OBELISK_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `obelisk/${InstallationVersion}`,
