...
Delete `packages/obelisk/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/obelisk/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/obelisk/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/obelisk`
- TUI plugin loader tests in `packages/obelisk` if runtime plugin API wiring changed
