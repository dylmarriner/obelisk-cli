---
title: Regeln
description: Lege eigene Anweisungen fuer obelisk fest.
---

Du kannst obelisk ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in obelisk den Befehl `/init` aus.
...
So kann obelisk sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht obelisk sie zu erweitern.
...
obelisk kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/obelisk/AGENTS.md` speichern.
Diese gelten in allen obelisk-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Obelisk dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/obelisk/AGENTS.md` existiert)
...
Beim Start sucht obelisk in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/obelisk/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `obelisk.json` oder in `~/.config/obelisk/obelisk.json` angeben.
...
obelisk parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit obelisk.json

Empfohlen ist das `instructions`-Feld in der `obelisk.json`:
...
Du kannst obelisk explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `obelisk.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
