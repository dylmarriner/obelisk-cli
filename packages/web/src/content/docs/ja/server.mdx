---
title: サーバー
description: HTTP 経由で Obelisk サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`obelisk serve` コマンドは、Obelisk クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `obelisk.local` |
...
HTTP 基本認証でサーバーを保護するには、`OBELISK_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `obelisk` になるか、`OBELISK_SERVER_USERNAME` を設定してオーバーライドします。これは、`obelisk serve` と `obelisk web` の両方に当てはまります。
...
`obelisk` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Obelisk サーバーを使用して、プログラムで Obelisk と対話します。
:::

`obelisk serve` を実行してスタンドアロンサーバーを起動できます。すでに Obelisk TUI を実行している場合でも、`obelisk serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Obelisk [IDE](/docs/ide) プラグインによって使用されます。
