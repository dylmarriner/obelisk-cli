...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("obelisk-find")
  api.delete("obelisk-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("obelisk-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("obelisk-find", new Highlight(...rest))
    return true
  }
