---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

Obelisk bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Obelisk-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/obelisk/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Obelisk-konfigurationen.
...
Obelisk Zen er en liste over modeller leveret af Obelisk-teamet, som er blevet testet og verificeret til at fungere godt med Obelisk. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Obelisk Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg obelisk og gå til [obelisk.ai/auth](https://obelisk.ai/auth).
...
Det fungerer som alle andre udbydere i Obelisk og er helt valgfrit at bruge.

---

## Obelisk Go

Obelisk Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Obelisk-teamet, som er testet og verificeret til at fungere godt med Obelisk.

1. Kør kommandoen `/connect` i TUI, vælg `Obelisk Go`, og gå til [obelisk.ai/auth](https://obelisk.ai/zen).
...
Sådan bruger du Amazon Bedrock med Obelisk:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører obelisk:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `obelisk.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Obelisk understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere obelisk til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at obelisk skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem obelisk-konfigurationen.
...
gemme tokenet i obelisk auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Obelisk bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Obelisk
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`obelisk.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `obelisk.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Obelisk:
...
Sådan bruger du Google Vertex AI med Obelisk:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem obelisk, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session) plugin for automatisk at logge hver Obelisk-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Obelisk-samtale opført som en separat session.
...
Du kan konfigurere obelisk til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere obelisk til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere obelisk til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Obelisk. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/obelisk) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Obelisk:
...
3. Kopier API-nøglen til brug i Obelisk.
...
6. **Vigtigt**: Før du bruger skymodeller i Obelisk, skal du hente modelinformationen lokalt:
...
Obelisk Zen er en liste over testede og verificerede modeller leveret af Obelisk-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Obelisk Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Obelisk Zen**.

   ```txt
   /connect
   ```

3. Indtast Obelisk API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem obelisk-konfigurationen.
...
5. Du kan også tilpasse dem gennem obelisk-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Obelisk:
...
Du kan også tilpasse modeller gennem obelisk-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med obelisk. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `obelisk.json`-filen i projektkataloget:
...
`limit`-felterne lader Obelisk forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `obelisk auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek obelisk-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i obelisk-konfigurationen.
