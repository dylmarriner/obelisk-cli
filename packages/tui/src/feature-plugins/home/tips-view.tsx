import type { TuiPluginApi } from "@obelisk-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("obelisk.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public obelisk.ai link",
  ...
  "Create {highlight}obelisk.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/obelisk/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.obelisk/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.obelisk/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.obelisk/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.obelisk/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Obelisk from reading sensitive files",
  "Use {highlight}obelisk run{/highlight} for non-interactive scripting",
  "Use {highlight}obelisk --continue{/highlight} to resume the last session",
  "Use {highlight}obelisk run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}obelisk serve{/highlight} for headless API access to Obelisk",
  "Use {highlight}obelisk run --attach{/highlight} to connect to a running server",
  "Run {highlight}obelisk upgrade{/highlight} to update to the latest version",
  "Run {highlight}obelisk auth list{/highlight} to see all configured providers",
  "Run {highlight}obelisk agent create{/highlight} for guided agent creation",
  "Use {highlight}/obelisk{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}obelisk github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/obelisk fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.obelisk/themes/{/highlight} directory",
  ...
  "Run {highlight}obelisk debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/obelisk{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with Obelisk Zen for curated, tested models",
