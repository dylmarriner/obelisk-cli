...
Puoi aggiungere strumenti esterni a Obelisk usando il _Model Context Protocol_, o MCP. Obelisk supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Obelisk Config](https://obelisk.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/obelisk`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Obelisk gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Obelisk:
...
Se il server richiede autenticazione, Obelisk ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `obelisk mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Obelisk memorizzera' i token in modo sicuro in `~/.local/share/obelisk/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Obelisk insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Obelisk come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Obelisk al tuo account Sentry.
