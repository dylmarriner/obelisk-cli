...
import { Event } from "@obelisk-ai/schema/event"
import { SessionStatusEvent } from "@obelisk-ai/schema/session-status-event"
import { SessionV1 } from "@obelisk-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Obelisk/TimelineStability"
...
export const model = { providerID: "obelisk", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("obelisk.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["obelisk"],
    default: { providerID: "obelisk", modelID: "claude-opus-4-6" },
  }
}
