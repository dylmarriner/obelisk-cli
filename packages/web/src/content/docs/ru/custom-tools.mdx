...
description: Создавайте инструменты, которые LLM может вызывать в obelisk.
---

Пользовательские инструменты — это создаваемые вами функции, которые LLM может вызывать во время разговоров. Они работают вместе со [встроенными инструментами ](/docs/tools) obelisk, такими как `read`, `write` и `bash`.
...
- Локально, поместив их в каталог `.obelisk/tools/` вашего проекта.
- Или глобально, поместив их в `~/.config/obelisk/tools/`.
