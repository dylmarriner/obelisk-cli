---
title: Serveur
description: Interagissez avec le serveur obelisk via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `obelisk serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client obelisk peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `obelisk.local` |
...
Définissez `OBELISK_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `obelisk`, ou définissez `OBELISK_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `obelisk serve` et à `obelisk web`.
...
Lorsque vous exécutez `obelisk`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur obelisk pour interagir avec obelisk par programmation.
:::

Cette architecture permet à obelisk de prendre en charge plusieurs clients et vous permet d'interagir avec obelisk par programme.

Vous pouvez exécuter `obelisk serve` pour démarrer un serveur autonome. Si vous avez le
obelisk TUI en cours d'exécution, `obelisk serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Obelisk [IDE](/docs/ide).
...
Le serveur obelisk expose les API suivantes.
