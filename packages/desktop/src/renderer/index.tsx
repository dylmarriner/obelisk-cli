...
import type { UpdaterState } from "@obelisk-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@obelisk-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OBELISK_CHANNEL === "prod" &&
...
const deepLinkEvent = "obelisk:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__OBELISK__ ??= {}
  const pending = window.__OBELISK__.deepLinks ?? []
  window.__OBELISK__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `obelisk.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://obelisk.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("obelisk.global.dat").getItem("language")
