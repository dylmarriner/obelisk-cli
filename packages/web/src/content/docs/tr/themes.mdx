...
obelisk ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak obelisk kendi `obelisk` temasını kullanır.
...
obelisk birden fazla yerleşik temayla gelir.
...
- obelisk'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
obelisk, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/obelisk/themes/*.json` veya `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.obelisk/themes/*.json`
4. **Geçerli çalışma dizini** - `./.obelisk/themes/*.json`
