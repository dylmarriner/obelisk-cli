...
import { SessionV1 } from "@obelisk-ai/core/v1/session"
import { ModelsDev } from "@obelisk-ai/core/models-dev"
import { HttpRecorder } from "@obelisk-ai/http-recorder"
import { HttpRecorderInternal } from "@obelisk-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@obelisk-ai/llm"
import { RequestExecutor } from "@obelisk-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.obelisk.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OBELISK_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["obelisk", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OBELISK_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OBELISK_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["obelisk", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "obelisk-proxy",
    name: "Obelisk proxy",
    providerID: ProviderV2.ID.obelisk,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["obelisk", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OBELISK_RECORD_CONSOLE_TOKEN && process.env.OBELISK_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.obelisk,
        name: "Obelisk Zen",
        env: ["OBELISK_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OBELISK_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OBELISK_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OBELISK_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["obelisk", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OBELISK_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OBELISK_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OBELISK_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OBELISK_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the obelisk LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "obelisk.json"),
      JSON.stringify({ $schema: "https://obelisk.ai/config.json", ...scenario.config(model) }),
    ),
  )
