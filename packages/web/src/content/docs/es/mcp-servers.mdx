...
Puede agregar herramientas externas a Obelisk utilizando el _Model Context Protocol_ o MCP. Obelisk admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Obelisk Config](https://obelisk.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/obelisk`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Obelisk maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Obelisk:
...
Si el servidor requiere autenticación, Obelisk le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `obelisk mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Obelisk almacenará los tokens de forma segura en `~/.local/share/obelisk/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Obelisk, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Obelisk como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Obelisk a su cuenta Sentry.
