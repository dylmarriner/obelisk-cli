---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Obelisk.
---

import config from "../../../../config.mjs"
export const console = config.console

Obelisk koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Obelisk konfiguraciji.
...
u `~/.local/share/obelisk/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Obelisk
config.
...
Obelisk Zen je lista modela koje je obezbedio Obelisk tim koji su bili
testirano i potvrđeno da dobro radi sa Obelisk. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Obelisk Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite obelisk i idite na [obelisk.ai/auth](https://obelisk.ai/auth).
...
Radi kao i svaki drugi provajder u Obelisk i potpuno je opcionalan za korištenje.

---

## Obelisk Go

Obelisk Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Obelisk tim i koji su testirani i verificirani da dobro rade s Obelisk-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Obelisk Go` i idite na [obelisk.ai/auth](https://obelisk.ai/zen).
...
Da biste koristili Amazon Bedrock s Obelisk:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete obelisk:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `obelisk.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Obelisk.
:::
...
Možete konfigurirati obelisk za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi obelisk ispravno radio.
   :::
...
Također možete dodati modele kroz svoju obelisk konfiguraciju.
...
da pohrani token u obelisk auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Obelisk koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Obelisk
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`obelisk.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [obelisk-gitlab-auth](https://www.npmjs.com/package/obelisk-gitlab-auth).

##### Konfiguracija

Prilagodite putem `obelisk.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s obelisk:
...
Za korištenje Google Vertex AI s Obelisk:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće obelisk.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem obeliska, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [obelisk-helicone-session](https://github.com/H2Shami/obelisk-helicone-session) da automatski prijavite svaki Obelisk razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Obelisk razgovor naveden kao zasebna sesija.
...
Možete konfigurirati obelisk za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati obelisk za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati obelisk za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Obelisk. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/obelisk) za detalje.
:::
...
Da biste koristili Ollama Cloud s Obelisk:
...
3. Kopirajte API ključ za korištenje u Obelisk.
...
6. **Važno**: Prije upotrebe modela oblaka u Obelisk, morate lokalno povući informacije o modelu:
...
Obelisk Zen je lista testiranih i verifikovanih modela koju je obezbedio Obelisk tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Obelisk Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Obelisk Zen**.

```txt
   /connect
```

3. Unesite svoj Obelisk API ključ.
...
Također možete dodati dodatne modele putem vaše obelisk konfiguracije.
...
5. Također ih možete prilagoditi putem vaše obelisk konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju obelisk konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s obelisk-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `obelisk.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Obelisk da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `obelisk auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Obelisk konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj obelisk konfiguraciji.
