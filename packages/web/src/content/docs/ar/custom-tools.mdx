...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في obelisk.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في obelisk مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.obelisk/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/obelisk/tools/`.
