...
O Obelisk Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Obelisk. Você assina o Obelisk Go e
...
O Obelisk Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Obelisk Go funciona como qualquer outro provedor no Obelisk.

1. Você entra no **<a href={console}>Obelisk Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Obelisk Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Obelisk Go.
:::
...
O Obelisk Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Obelisk
usa o formato `obelisk-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`obelisk-go/kimi-k2.7-code` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Obelisk também.
