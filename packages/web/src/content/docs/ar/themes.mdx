...
مع Obelisk يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Obelisk سمتنا `obelisk`.
...
يأتي Obelisk مع عدة سمات مدمجة.
...
- يريدون أن يطابق Obelisk مظهر terminal لديهم
...
يدعم Obelisk نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/obelisk/themes/*.json` أو `$XDG_CONFIG_HOME/obelisk/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.obelisk/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.obelisk/themes/*.json`
