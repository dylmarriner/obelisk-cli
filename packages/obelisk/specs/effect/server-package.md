...
Practical reference for a future `packages/server` split after the obelisk
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/obelisk`.
...
Until enough shared service code lives outside `packages/obelisk`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/obelisk`

It should not import `packages/obelisk` services while `packages/obelisk`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on obelisk-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/obelisk` implementation details.
