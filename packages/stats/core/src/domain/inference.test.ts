...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve obelisk route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "obelisk")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "obelisk")).toBe("openai")
    expect(statProvider("big-pickle", "", "obelisk")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "obelisk"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep obelisk as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "obelisk"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "obelisk"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep obelisk or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "obelisk"), country: "US" })).toMatchObject([
