...
Agent Skills erlauben Obelisk, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Obelisk sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Obelisk vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.obelisk/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/obelisk/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.obelisk/skills/git-release/SKILL.md` zum Beispiel so:
...
Obelisk listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `obelisk.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `obelisk.json`):
