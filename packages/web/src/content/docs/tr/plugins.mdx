---
title: Eklentiler
description: Obelisk'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Obelisk'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Obelisk'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.obelisk/plugins/` - Proje düzeyinde eklentiler
- `~/.config/obelisk/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/obelisk/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/obelisk/obelisk.json`)
2. Project config (`obelisk.json`)
3. Global eklenti dizini (`~/.config/obelisk/plugins/`)
4. Proje eklenti dizini (`.obelisk/plugins/`)
...
Obelisk bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Obelisk SDK istemcisi.
...
Obelisk'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Obelisk masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Obelisk'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Obelisk'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Obelisk'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Obelisk'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://obelisk.ai/docs/sdk) bakın.
