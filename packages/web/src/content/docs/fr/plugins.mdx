---
title: Plugins
description: Écrivez vos propres plugins pour étendre Obelisk.
---

Les plugins vous permettent d'étendre Obelisk en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Obelisk.
...
- `.obelisk/plugins/` - Plugins au niveau du projet
- `~/.config/obelisk/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/obelisk/node_modules/`.
...
1. Configuration globale (`~/.config/obelisk/obelisk.json`)
2. Configuration du projet (`obelisk.json`)
3. Répertoire global des plugins (`~/.config/obelisk/plugins/`)
4. Répertoire des plugins du projet (`.obelisk/plugins/`)
...
Obelisk exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK obelisk pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre obelisk.
...
:::note
Si vous utilisez l'application de bureau Obelisk, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez obelisk de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à obelisk :
...
L'assistant `tool` crée un outil personnalisé que obelisk peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour obelisk aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://obelisk.ai/docs/sdk) pour plus de détails.
